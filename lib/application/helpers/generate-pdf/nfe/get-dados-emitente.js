...
async function getDadosEmitente({ y, doc, ajusteX, ajusteY, margemDireita, margemEsquerda, margemTopo, larguraDoFormulario, emit, protNFe, pathLogo, ide, folha, emailEmitente, celularEmitente }) {
    doc
        .lineWidth(0.5)
    ...
    if (pathLogo !== undefined) {
        if (folha === 0) {
            doc.image(pathLogo, margemEsquerda + ajusteX + 1.4, margemTopo + ajusteY + default_1.DEFAULT_NFE.ajusteYDoLogotipo + 68, {
                fit: [90, 90]
            });
        }
        else {
    ...
    if (emit.enderEmit && emit.enderEmit.fone !== undefined) {
        ...
        const foneTexto = celularEmitente
            ? `${(0, utils_1.formatPhone)(tel.trim())} / ${(0, utils_1.formatPhone)(celularEmitente.trim())}`
            : (0, utils_1.formatPhone)(tel.trim());
        (0, normal_1.normal)({
            doc,
            value: foneTexto,
            x: identificacaoDoEmitenteX,
